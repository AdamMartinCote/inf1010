...
void Gamebay::setUI(){
    ...
    QGridLayout* layoutAttaques = new QGridLayout();
    layoutAttaques->addWidget(choixAttaque_);
    ...
    layoutEcran->addLayout(layoutAttaques);
...
void Gamebay::setConnections(){
    ...
    //!!!!!! A COMPLETER !!!!!!
    QObject::connect(boutonStart_, SIGNAL(clicked(bool)), menu_, SLOT(gestionDuMenu()));
...
void Gamebay::afficherCreaturesDresseur(){
    //remplit l’objet QListWidget avec la liste des créatures que vous possédez
...
void Gamebay::debuterCombat(Creature* creature){
    ...
    for(unsigned int i = 0; i< polyland_->obtenirHero().obtenirCreatures().size(); i++){
...
void Gamebay::attaquerCreatureAdverse(){
    ...
    try{
         ...
         if(creatureAdverse_->obtenirPointDeVie() <= 0) emit creatureAdverseVaincue();

    }catch(ExceptionCreatureMorte e){

        QMessageBox messageException;

        if(e.obtenirValeurCompteur() < 3)
            messageException.setText("Arretez c'est deja fini !");
        else if (e.obtenirValeurCompteur() < 5){
            messageException.setText("Hey ca fait deja "
            + QString::number(e.obtenirValeurCompteur()) + "e fois que vous essayez...");
        }
        else
            messageException.setText("Vous dis donc... Vous etes vraiment sadique a vous "
                                     "acharner sur une pauvre creature deja morte");
        messageException.exec();

    }catch(ExceptionAttaqueEchouee e){
        QMessageBox msgbox;
        msgbox.setText("Attaque echoue");
        msgbox.exec();
    }
...
// capturerCreatureAdverse == attraperCreatureAdverse
void Gamebay::capturerCreatureAdverse(){
    //!!!!!! A COMPLETER !!!!!!
    // le controle des doublons est geré par la classe Dresseur
    try{
            polyland_->attraperCreature(&polyland_->obtenirHero(), creatureAdverse_);
    }
    catch(ExceptionEchecCapture e){
        QMessageBox msg;
        msg.setText("Vous possdez deja cette creature");
        msg.exec();
    }
}
