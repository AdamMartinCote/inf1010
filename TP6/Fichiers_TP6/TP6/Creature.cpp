...
// Lance un exception si la creature est morte ou si l'attaque echoue
// par manque d'energie
void Creature::attaquer(const Pouvoir & pouvoir, Creature & creature)
{
	...
	if (energie_ >= pouvoir.obtenirEnergieNecessaire() && pouvoirEstDansVector)
	{
        ...
        else if(creature.obtenirPointDeVie() <= 0) throw ExceptionCreatureMorte();

	}
    else if (energie_ < pouvoir.obtenirEnergieNecessaire()) throw ExceptionAttaqueEchouee();
...
int Creature::experienceGagnee(const Creature& creature)
{
	...
	if (creature.obtenirPointDeVie() <= 0) {
        ...
        if ((unsigned int)experience > (experienceNecessaire_ - experience_)) {
