...
{
        //list<T*>::iterator
        auto pos = find(listMaitre_.begin(), listMaitre_.end(), t);
...
{
        /*list<S*>::iterator pos*/ auto pos = find(listCompagnon_.begin(), listCompagnon_.end(), s);
...
bool MondeMagique<T, S>::supprimerCompagnon(const Predicate& predicate) {
	size_t tailleAvant = listCompagnon_.size();
        //list<S*>::iterator
        auto pos = find_if(listCompagnon_.begin(), listCompagnon_.end(), predicate);
