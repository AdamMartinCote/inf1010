...
// prototypes
template<class FuncType> void supprimerElements(FuncType foncteur);
...
int main()
{
    ...
    vous.supprimerElements(std::bind(std::greater<int>(), std::bind(obtenirAttaque, std::placeholders::_1), attaqueMax);
	
	// http://stackoverflow.com/questions/22422147/why-is-stdbind-not-working-without-placeholders-in-this-example-member-functi
	...
	// A COMPLETER...
	creaturesInscrites.erase(&sacha, creaturesInscrites.second);
	...
	// A COMPLETER...	
	vous.supprimerElements(FoncteurCreaturesDeMemeNom(touflamme));
	sacha.supprimerElements(FoncteurCreaturesDeMemeNom(touflamme));
	polyland.supprimerCompagnon(&touflamme);
