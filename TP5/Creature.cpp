...
{
//    for each (Pouvoir* pouvoir in  creature.pouvoirs_)	This non-standard keyword is available in both C++/CLI and native C++ projects. 
//    {														However, its use is not recommended. Consider using a standard Range-based for 
//        pouvoirs_.push_back(new Pouvoir(*pouvoir));		Statement (C++) instead.
//    }
	for (auto pouvoir : creature.pouvoirs_){
		pouvoirs_.push_back(new Pouvoir(*pouvoir));
	}
}
...
Creature& Creature::operator=(const Creature& creature)
{
	if (this != &creature)
	{
		...
		for (auto i : creature.pouvoirs_) {
			pouvoirs_.push_back(new Pouvoir(*i));
		}
	}
	return *this;
}
