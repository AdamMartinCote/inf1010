...
{
	pointDeVieTotal_ = 0;	// ajouté; erreur?
	energieTotal_ = 0;
...
int Creature::experienceGagnee(const Creature& creature)
{
	if(creature.obtenirPointDeVie() <= 0)
	{
		...
		{
			unsigned int experienceRestante = experience - (experienceNecessaire_ - experience_);
			niveau_++;
			attaque_ += 1;
			defense_ += 1;
			pointDeVie_ += 5;
...
std::string Creature::obtenirTypeCreature() const
{
	...
	std::string nomTypeCreature = typeid(*this).name();
	// effacer les caractères numérique de la string
	// sinon on a "15CreatureMagique"
	nomTypeCreature.erase(std::remove_if(nomTypeCreature.begin(), nomTypeCreature.end(), &isdigit), nomTypeCreature.end());
	return nomTypeCreature;
}
