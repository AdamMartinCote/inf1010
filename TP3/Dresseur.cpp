...
Dresseur::Dresseur()
	...
	, equipe_("")
{
	objetMagique_ = new ObjetMagique();
};
	...
	, equipe_(dresseur.equipe_)
{
	...
	for(auto i : dresseur.creatures_)
	{
		creatures_.push_back(new Creature(*i));
	}
}
...
{
	return os << dresseur.nom_ << " possede " << dresseur.creatures_.size() << " creature(s) et appartient a l'equipe "
	       << dresseur.equipe_ << std::endl;
}
...
Dresseur& Dresseur::operator=(const Dresseur& dresseur)
{
	nom_ = dresseur.nom_;
	equipe_ = dresseur.equipe_;
	modifierObjetMagique(dresseur.obtenirObjetMagique());

	for(int i = creatures_.size() - 1; i >= 0; i--)
	{
		delete creatures_[i];
		creatures_.pop_back();
	}
	for(auto i : dresseur.obtenirCreatures())
	{
		creatures_.push_back(new Creature(*i));
	}
	return (*this);
